...
Real Gemini API runner for Week 4 data collection, reading the full
220-prompt, 10-category curated dataset. This is one of three decoupled
provider runners (alongside run_mistral_benchmark.py and
run_groq_benchmark.py), each writing to its own partitioned log directory.

STOCHASTIC RELIABILITY SAMPLE: 20 of the 220 prompts are marked
"stochastic_sample": true with "repeat_count": 5 in prompts.json (per the
proposal's Reliability requirement). Those 20 prompts are called 5 times
each, producing 5 separately-saved response files per prompt
(gemini_P037_run1.json ... gemini_P037_run5.json), so the run-to-run
variation in the model's answers can be measured later (mean, median,
standard deviation, coefficient of variation). The remaining 200 prompts are
called once, exactly as in the original single-pass design. This means a
full run makes 300 total API calls (200 x 1 + 20 x 5), not 220.

MODEL: gemini-3.5-flash-lite - Google's GA, high-volume/low-latency model
(confirmed via Google's own release notes), used specifically because the
previous model (gemini-3.5-flash) was found to enforce a strict 20
requests/day free-tier cap. Flash-Lite is explicitly positioned by Google
for "high-volume automation," making it a better fit here. If it turns out
to have a similarly restrictive cap, check ai.google.dev for current limits.

SAFETY MECHANISMS: proactive throttling, reactive backoff+jitter,
persistent-error detection with early stop (covers quota exhaustion and
model retirement/unavailability - this project has hit both), atomic cache
writes, and resume-by-cache-file (works per-run for the repeated prompts,
per-prompt for everything else).
...
HOW TO RUN FOR REAL (VS Code):
  ...
  4. If it stops early due to quota, re-run later - it resumes exactly
     where it stopped, including partially-completed repeat runs.
"""
...
BASE_DELAY = 1.0
MAX_DELAY = 60.0
MIN_INTERVAL = 4.5  # seconds - conservative default; lower if Flash-Lite's real limit proves higher
...
class GeminiBenchmarkRunner:
    ...
    def _cache_path(self, prompt_id: str, run_number: int = None) -> str:
        """Single-pass prompts use gemini_P001.json. Repeated prompts use
        gemini_P037_run1.json, gemini_P037_run2.json, etc."""
        if run_number is None:
            return os.path.join(self.log_dir, f"gemini_{prompt_id}.json")
        return os.path.join(self.log_dir, f"gemini_{prompt_id}_run{run_number}.json")
    ...
    def _score_one_call(self, item, run_number, completed, total, skipped):
        """Handles exactly one API call (one run of one prompt), including
        error handling. Returns 'completed', 'skipped', or 'stop'."""
        prompt_id = item["id"]
