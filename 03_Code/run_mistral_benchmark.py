...
Real Mistral API runner for Week 4 data collection, reading the full
...
per prompt (mistral_P037_run1.json ... mistral_P037_run5.json), so run-to-run
...
MODEL: open-mistral-nemo - Mistral's recommended free-tier starting model.
...
class MistralBenchmarkRunner:
    ...
    def _cache_path(self, prompt_id: str, run_number: int = None) -> str:
        ...
        return os.path.join(self.log_dir, f"mistral_{prompt_id}_run{run_number}.json")
    ...
    def _score_one_call(self, item, run_number, completed, total, skipped):
        ...
        try:
            ...
            if systemic:
                ...
                logger.error(
                    "STOPPING RUN EARLY on %s%s. %d completed, %d remaining. %s. Re-run to resume.",
